		...
		if (in_array($fileExt, $allowed)){
			if ($Error === 0) {
				if($imageSize < 10000000) {	
					...
					//Getting and setting $user_id, this will be stored with the image
					$user = new User(); 
					$user_id = escape($user->data()->id);

					//littermate id number			
					$hatchlingIdNo = $_GET['link'];

					//get * from littermates where id = speciesId 
					$sq = 'SELECT * FROM littermates WHERE id = :id';
					$st = $pdo->prepare($sq);
					$st->execute(['id' => $hatchlingIdNo]);
					$com = $st->fetch();
					
					//setting veribales
					$speciesid = $com->speciesId;
					$morph = $com->morph;
					$double_gene = $com->doubleMorph;
					$locality = $com->locality;
					$mixed_locality = $com->mixedLocality;
					...
					if (empty($morph) && empty($double_gene) && empty($locality) && empty($mixed_locality)){
						echo  'This hatchling needs more info before continuing.' 
						//need to redirect later
						//edirect::to(BASE_URL . 'hatchling.php?link=' . $hatchlingIdNo ); 
					}
					if (!empty($morph) && empty($double_gene) && empty($locality) && empty($mixed_locality)){
						$alt = ucfirst($morph) . " " . ucfirst($co) . " " . ucfirst($la);
					}
					if (empty($morph) && !empty($double_gene) && empty($locality) && empty($mixed_locality)){
						$alt = ucfirst($double_gene) . " " . ucfirst($co) . " " . ucfirst($la);
					}
					if (empty($morph) && empty($double_gene) && !empty($locality) && empty($mixed_locality)){
						$alt = ucfirst($locality) . " " . ucfirst($co) . " " . ucfirst($la);
					}
					if (empty($morph) && empty($double_gene) && empty($locality) && !empty($mixed_locality)){
						$alt = ucfirst($mixed_locality) . " " . ucfirst($co) . " " . ucfirst($la);
					}
					if (!empty($morph) && empty($double_gene) && !empty($locality) && empty($mixed_locality)){
						$alt = ucfirst($morph) . " " . ucfirst($locality) . " " . ucfirst($co) . " " . ucfirst($la);
					}
					if (!empty($morph) && empty($double_gene) && empty($locality) && !empty($mixed_locality)){
						$alt = ucfirst($morph) . " " . ucfirst($mixed_locality) . " " . ucfirst($co) . " " . ucfirst($la);
					}
					if (empty($morph) && !empty($double_gene) && !empty($locality) && empty($mixed_locality)){
						$alt = ucfirst($double_gene) . " " . ucfirst($locality) . " " . ucfirst($co) . " " . ucfirst($la);
					}
					if (empty($morph) && !empty($double_gene) && empty($locality) && !empty($mixed_locality)){
						$alt = ucfirst($double_gene) . " " . ucfirst($mixed_locality) . " " . ucfirst($co) . " " . ucfirst($la);
					}
					...
					if(isset($_POST['submit'])){
						if($validation->passed()){
							try{							
								...
								$sq = 'INSERT INTO images(image_address, hatchId, speciesid, morph, double_gene, locality, mixed_locality, alt, LMId, status, added_by_user_id, image) VALUES(:image_address, :hatchId, :speciesid, :morph, :double_gene, :locality, :mixed_locality, :status, :alt, :LMId, :added_by_user_id, :image)';
								$st = $pdo->prepare($sq);
								$st->execute(['image_address' =>  $imageDestination, 'hatchId' => $hatchlingIdNo, 'speciesid' => $speciesid, 'morph' => $morph, 'double_gene' => $double_gene, 'locality' => $locality, 'mixed_locality' => $mixed_locality,	'alt' => $atl,	'LMId' => $hatchlingIdNo,	'status' => Input::get('status'), 'added_by_user_id' => $user_id, 'image' => $imagename]);
								// echo 'Post Added';
								echo $imageDestination . '<br>';
								echo $hatchlingIdNo . '<br>';
								echo $speciesid . '<br>';
								echo $morph . '<br>';
								echo $double_gene . '<br>';
								echo $locality . '<br>';
								echo $mixed_locality . '<br>';
								echo $atl . '<br>';
								echo $hatchlingIdNo . '<br>';
								echo Input::get('status') . '<br>';
								echo $user_id . '<br>';
								echo $imagename;
										...
										<div>
											<table>
												...
												<tbody>
													    ...
													    $imagesql = 'SELECT * FROM images WHERE hatchId = :hatchId';;
														$imagestmt = $pdo->prepare($imagesql);
														$imagestmt->execute(['hatchId' => $hatchlingIdNo]);
