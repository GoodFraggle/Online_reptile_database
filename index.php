					...
					<div class="main-page main-page-2">
						<h3>Species list</h3>
							<table>
								...
								<tbody>
									<?php
										$speci = DB::getInstance()->query("SELECT * FROM species ORDER BY common ASC");
										foreach($speci->results() as $speci){ 
									?>
										<tr>
											<td><?php echo escape(ucfirst($speci->id));?></td>
											<td> <?php echo escape(ucfirst($speci->common)); ?></td>
											<td> <?php echo escape(ucfirst($speci->latin)); ?></td>			
										</tr>
									<?php } ?>	
								</tbody>
							</table>
