...
//check if input fields are set
    ...
    if($validation->passed()){
        ...
        //Create a new user and inserting the info in user table.
			...
			$sql = 'INSERT INTO mixed_locality(name, speciesId, locality1, locality2, adult_size, hatchling_size, comments) VALUES(:name, :speciesId, :locality1, :locality2, :adult_size, :hatchling_size, :comments)';
			$stmt = $pdo->prepare($sql);
			$stmt->execute(['name' => Input::get('name'), 'speciesId'=> Input::get('speciesId'), 'locality1'=> Input::get('locality1'), 'locality2'=> Input::get('locality2'), 'adult_size'=> Input::get('adult_size'), 'hatchling_size'=> Input::get('hatchling_size'), 'comments'=> Input::get('comments') ]);
								...
								<form action="" method="POST" autocomplete="off">
									<fieldset>
										...
										<div class="form-2-collums form-2-1">
											...
											<select id="locality1" name="locality1">
												<option> Choose a locality </option>
												<?php 
												$local = DB::getInstance()->query("SELECT * FROM locality");
												foreach($local->results() as $local){?> 
													<option value="<?php echo $local->id; ?>"><?php echo $local->name; ?></option>
												<?php } ?>				
											</select>
											<select id="locality2" name="locality2">
												<option> Choose a second locality </option>
												<?php 
												$local = DB::getInstance()->query("SELECT * FROM locality");
												foreach($local->results() as $local){?> 
													<option value="<?php echo $local->id; ?>"><?php echo $local->name; ?></option>
